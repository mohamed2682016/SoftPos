...
namespace SoftPos.Services
{
    ...
    .ToList();
        ...
        public async Task<bool> AddSalesInvoiceAsync(SalesMaster model)
        {
            try
            {
                // Set default values
                model.SerialNo = await GetSerialNo();
                model.VoucherTypeId = 1; // Default sales voucher type
                model.Status = "Active";
                model.PaymentStatus = "Unpaid";
                model.FinancialYearId = 1; // Default financial year
                model.AddedDate = DateTime.UtcNow;
                model.TenantId = "1";
                
                // Copy SalesDetails to listOrder for processing
                model.listOrder = model.SalesDetails ?? new List<SalesDetails>();
                
                // Save the invoice
                int result = await SaveAsync(model);
                return result > 0;
            }
            catch (Exception ex)
            {
                throw new Exception("An error occurred while adding the sales invoice. Please try again.", ex);
            }
        }
