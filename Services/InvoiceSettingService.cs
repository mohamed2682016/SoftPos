...
namespace SoftPos.Services
{
    public class InvoiceSettingService
    {
        ...
        public async Task<InvoiceSetting> GetInvoiceSettingAsync(int voucherTypeId)
        {
            var setting = await _database.Table<InvoiceSetting>()
                .FirstOrDefaultAsync(i => i.VoucherTypeId == voucherTypeId);
            
            return setting ?? new InvoiceSetting 
            { 
