...
namespace SoftPos.Services
{
    public class AccountLedgerService
    {
        ...
        public async Task<List<AccountLedgerView>> GetAllAccountLedgersByIdAsync(int accountGroupId)
        {
            ...
            {
                var customerPostings = ledgerPostings.Where(lp => lp.LedgerId == customer.LedgerId).ToList();
                decimal totalDebit = customerPostings.Sum(lp => lp.Debit);
                decimal totalCredit = customerPostings.Sum(lp => lp.Credit);
                
                // For customers (AccountGroupId = 26), positive balance means they owe money (debt)
                customer.CurrentBalance = totalDebit - totalCredit;
            }
