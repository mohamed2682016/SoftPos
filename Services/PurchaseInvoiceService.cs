    ...
    .ToListAsync();
        ...
        public async Task<int> SaveAsync(PurchaseMaster model)
        {
            ...
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    // If there is a paid amount, create a payment record
                    if (model.PayAmount > 0)
                    {
                        await CreatePaymentRecordAsync(model);
                    }
        ...
        public async Task<bool> UpdateAsync(PurchaseMaster model)
        {
            ...
            try
            {
                ...
                // Handle payment update if PayAmount changed
                var existingPayments = await _database.Table<PaymentDetails>()
                    .Where(pd => pd.PurchaseMasterId == model.PurchaseMasterId)
                    .ToListAsync();
                
                decimal totalPaidBefore = existingPayments.Sum(p => p.ReceiveAmount);
                
                // If new payment amount is different from existing, handle the difference
                if (model.PayAmount > totalPaidBefore && model.PayAmount > 0)
                {
                    decimal additionalPayment = model.PayAmount - totalPaidBefore;
                    if (additionalPayment > 0)
                    {
                        var tempModel = new PurchaseMaster
                        {
                            PurchaseMasterId = model.PurchaseMasterId,
                            VoucherNo = model.VoucherNo,
                            Date = model.Date,
                            ...
                            PayAmount = additionalPayment,
                            BalanceDue = model.BalanceDue,
                            UserId = model.UserId,
                            FinancialYearId = model.FinancialYearId
                        };
                        await CreatePaymentRecordAsync(tempModel);
                    }
                }
        ...
        private async Task CreatePaymentRecordAsync(PurchaseMaster model)
        {
            // Get payment serial number
            const string query = @"SELECT IFNULL(MAX(CAST(SerialNo AS INTEGER) + 1), 1) AS VoucherNo FROM PaymentMaster";
            var paymentSerialNo = await _database.ExecuteScalarAsync<string>(query);

            // Get invoice setting for payment out (VoucherTypeId = 10)
            var invoiceSetting = await _database.Table<InvoiceSetting>()
                                                .FirstOrDefaultAsync(x => x.VoucherTypeId == 10);

            string paymentVoucherNo = $"{invoiceSetting?.Suffix ?? string.Empty}-{paymentSerialNo}-{invoiceSetting?.Prefix ?? string.Empty}";

            // Create PaymentMaster record
            var paymentMaster = new PaymentMaster
            {
                VoucherNo = paymentVoucherNo,
                SerialNo = paymentSerialNo,
                Date = model.Date,
                LedgerId = model.LedgerId,
                Amount = model.PayAmount,
                Type = "PaymentOut",
                Narration = $"دفعة تلقائية من فاتورة مشتريات {model.VoucherNo}",
                UserId = model.UserId,
                FinancialYearId = model.FinancialYearId,
                VoucherTypeId = 10, // Payment Out
                Status = "Completed",
                AddedDate = DateTime.UtcNow
            };

            await _database.InsertAsync(paymentMaster);
            int paymentMasterId = paymentMaster.PaymentMasterId;
            ...
            if (paymentMasterId > 0)
            {
                ...
                var paymentDetails = new PaymentDetails
                {
                    ...
                    LedgerId = 1, // Cash account (assuming 1 is cash)
                    PurchaseMasterId = model.PurchaseMasterId,
                    TotalAmount = model.GrandTotal,
                    ReceiveAmount = model.PayAmount,
                    DueAmount = model.BalanceDue
                };

                await _database.InsertAsync(paymentDetails);

                // Create LedgerPosting for Supplier (Debit)
                var supplierPosting = new LedgerPosting
                {
                    Date = model.Date,
                    LedgerId = model.LedgerId,
                    Debit = model.PayAmount,
                    Credit = 0,
                    VoucherNo = paymentVoucherNo,
                    DetailsId = paymentMasterId,
                    YearId = model.FinancialYearId,
                    InvoiceNo = paymentVoucherNo,
                    VoucherTypeId = 10,
                    LongReference = paymentMaster.Narration,
                    ReferenceN = paymentMaster.Narration,
                    ChequeNo = string.Empty,
                    ChequeDate = string.Empty,
                    AddedDate = DateTime.UtcNow
                };

                await _database.InsertAsync(supplierPosting);

                // Create LedgerPosting for Cash (Credit)
                var cashPosting = new LedgerPosting
                {
                    Date = model.Date,
                    LedgerId = 1, // Cash account
                    Debit = 0,
                    Credit = model.PayAmount,
                    VoucherNo = paymentVoucherNo,
                    DetailsId = paymentMasterId,
                    YearId = model.FinancialYearId,
                    InvoiceNo = paymentVoucherNo,
