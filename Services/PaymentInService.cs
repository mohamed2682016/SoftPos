                ...
                foreach (var item in model.listOrder)
                {
                    ...
                    if (item.LedgerId > 0)
                    {
                        ...
                        if (item.SalesMasterId > 0)
                        {
                            ...
                            // Update PayAmount and BalanceDue - always recalculate regardless
                            decimal oldPayAmount = master.PayAmount;
                            if (oldPayAmount == 0)
                            {
                                master.PayAmount = item.ReceiveAmount;
                            }
                            else if (oldPayAmount != item.ReceiveAmount)
                            {
                                // Add to existing payment
                                master.PayAmount = oldPayAmount + item.ReceiveAmount;
                            }
                            // Note: If oldPayAmount == item.ReceiveAmount, payment was already recorded, 
                            // but we still need to update BalanceDue to ensure it's correct
                            
                            // Always update BalanceDue to ensure it's correct
                            master.PreviousDue = (master.GrandTotal) - master.PayAmount;
                            master.BalanceDue = (master.GrandTotal) - master.PayAmount;
                            
                            // Update status based on balance (keep PaymentStatus unchanged)
                            if (master.BalanceDue == 0)
                            {
                                master.Status = "Paid";
                                // Don't change PaymentStatus - keep original payment method (Cash, Card, UPI, etc.)
                            }
                            else
                            {
                        ...
                        cashPosting.Debit = item.ReceiveAmount; // Use individual receive amount, not total model.Amount
